...
pay_method_service.add = async function (customer_account_id, one_time_token, user) {
    try {
        ...
        if (customer) {
            ...
            throw { type: 'custom', status: 404, errors: null };
        }
    }
    catch (err) {
        throw err;
    ...
    try {
        ...
        if (pay_methods && pay_methods.length > 0) {
            ...
            if (found_pay_method) {
                ...
                throw { type: 'custom', status: 401, errors: 'Pay method id: ' + pay_method_id + ' does not belong to customer with account id: ' + customer_account_id };
            }
        }
        else {
            throw { type: 'custom', status: 401, errors: 'Pay method id: ' + pay_method_id + ' does not belong to customer with account id: ' + customer_account_id };
    ...
    try {
        ...
        if (pay_method) {
            if (pay_method.Type === 'CreditCard') {
                ...
                throw { type: 'custom', status: 400, errors: ['Cannot update the CVV and expiration of an eCheck pay method'] };
            }
        }
        else {
            throw { type: 'custom', status: 401, errors: 'Pay method id: ' + pay_method_id + ' does not belong to customer with account id: ' + customer_account_id };
