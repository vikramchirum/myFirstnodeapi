...
service_account_service.get_notes = async function (id, count) {
    try {
        let result = await await external_api.get('/api/service_accounts/' + id + '/notes?maxRetRec=' + count, false);
        return external_api.clean_response(result);
    }
    catch (err) {
        throw err;
...
service_account_service.post_notes = async function (id, note_req) {
    try {
        ...
        let result = await await external_api.post('/api/Notes/CSPNote', request);
        return external_api.clean_response(result);
    }
    catch (err) {
        throw err;
